...
# --- Patient list ---
patients = ["Deepika", "Taylor", "Kendall", "Gigi", "Ariana"]
...
def patient_sensor_stream(patients):
    """Simulates continuous streaming of sensor data for patients."""
    while True:
        data = []
        for p in patients:
            reading = {
...
# --- Run live updates ---
placeholder = st.empty()  # wrapper to allow safe reruns
with placeholder.container():
    for readings in patient_sensor_stream(patients):
        for reading in readings:
            ...
            temp_bar_value = clamp((temp - 95) * 25)  # scale temp 95–99.5°F → 0–100
